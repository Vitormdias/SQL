...
CREATE OR REPLACE TRIGGER ADD_PONTOS
	...
	BEGIN
		...
		VDIARIAS := :NEW.DATA_DEVOLUCAO - :OLD.DATA_LOCACAO; 
		...
		IF VCOD_CATEGORIA = 1 THEN
			VPTS_FIDELIDADE := VPTS_FIDELIDADE + (1* VDIARIAS);
		ELSIF VCOD_CATEGORIA = 2 THEN
			VPTS_FIDELIDADE := VPTS_FIDELIDADE + (2* VDIARIAS);
		ELSIF VCOD_CATEGORIA = 3 THEN
			VPTS_FIDELIDADE := VPTS_FIDELIDADE + (3* VDIARIAS);
		ELSIF VCOD_CATEGORIA = 4 THEN
			VPTS_FIDELIDADE := VPTS_FIDELIDADE + (4* VDIARIAS);
		END IF;
